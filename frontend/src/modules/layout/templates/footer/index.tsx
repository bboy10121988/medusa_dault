...
export default async function Footer() {
  ...
  // 從 Sanity 獲取 footer 資料，如果失敗則使用預設值
  let footer: any = null
  try {
      ...
      <div className="max-w-[1440px] mx-auto px-5 md:px-10">
        <div className="py-12 md:py-20">
          ...
          <div className="grid grid-cols-2 md:grid-cols-6 gap-y-10 md:gap-x-4">
              ...
              <LocalizedClientLink href="/" className="block">
                <div className="w-40">
                  {footer?.logo ? (
                    <img
                      src={footer.logo.url}
                      alt={footer.logo.alt || "Store logo"}
                      width={160}
                      height={60}
                      className="w-full"
                    />
                  ) : (
                    <div className="text-2xl font-bold">Medusa Store</div>
                  )}
                </div>
              </LocalizedClientLink>
            </div>
            ...
            {/* 商品系列 */}
            <div className="space-y-6">
              <h3 className="font-semibold text-lg">商品系列</h3>
            ...
            {/* 商品分類 */}
            <div className="space-y-6">
              <h3 className="font-semibold text-lg">商品分類</h3>
              <ul className="space-y-4">
                {productCategories?.slice(0, 3).map((category: any, index: number) => (
